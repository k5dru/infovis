...
select 
date_Trunc('DAY', timestamp) as utc_day, 
date_trunc('HOUR', timestamp) as utc_hour,
...
( 
  select wspr.*, 
  ntile(4) over (partition by substr(tx_grid, 1, 4), date_trunc('HOUR', timestamp) order by frequency) 
    as grid_frequency_ntile
  from wspr
  where timestamp between '2017-12-03 00:00' and '2017-12-03 23:59'
) wspr_ntile
group by 1,2,3
having count(*) > 3
order by 1,2;
